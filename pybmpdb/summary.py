...
def filterlocation(location, count=5, column="bmp"):
    location.filtered_data = location.filtered_data.groupby(level=column).filter(
        lambda g: g.count() >= count
    )

    location.include = (
        location.filtered_data.index.get_level_values(column).unique().shape[0]
    ) >= count
...
class DatasetSummary(object):
    ...
    def latex_file_name(self):
        if self._latex_file_name is None:
            self._latex_file_name = utils.processFilename(
                "{}_{}_{}".format(self.paramgroup, self.bmp, self.parameter.name)
            ).lower()
    ...
    def scatter_fig_name(self):
        if self._scatter_fig_name is None:
            figname = utils.processFilename(
                "{}_scatter.pdf".format(self.latex_file_name)
            )
    ...
    def _tex_table_row(
        ...
        if fromdataset:
            if self.ds.effluent.include and self.ds.influent.include:
                val = wqio.utils.sigFigs(
                    getattr(self.ds, attribute),
                    sigfigs,
                    pval=pval,
                    tex=tex,
                    forceint=forceint,
            ...
            for loc in [self.ds.influent, self.ds.effluent]:
                if loc.include:
                    if hasattr(attribute, "append"):
                        val = [getattr(loc, attr) for attr in attribute]
                    else:
                    ...
                    if val is not None:
                        if twoval:
                            ...
                            thisstring = wqio.utils.sigFigs(
                                val, sigfigs, pval=pval, tex=tex, forceint=forceint
                            )
            ...
            formatter = dict(
                ruler=thisrule, name=name, val_in=valstrings[0], val_out=valstrings[1]
            )
            row = r"""
                {ruler}
                {name} & {val_in} & {val_out} \\"""
...
class CategoricalSummary(object):
    def __init__(
        ...
        self._raw_datasets = [
            ds for ds in filter(lambda x: x.effluent.include, datasets)
        ]
    ...
    def datasets(self):
        if self._applyfilters:
            ...
            for ds in self._raw_datasets:
                filterlocation(
                    ds.effluent, count=self.filtercount, column=self.filtercolumn
                )

                filterlocation(
                    ds.influent, count=self.filtercount, column=self.filtercolumn
                )
    ...
    def makeReport(
        self, templatepath, inputpath, reportpath, report_title, regenfigs=True
    ):
...
def categorical_stats(dc, simple=False):
    return (
        dc.data.loc[:, dc.groupcols + ["bmp_id"]]
        .drop_duplicates()
        ...
        .assign(
            diff_medianci=~wqio.utils.checkIntervalOverlap(
                dc.median["inflow"], dc.median["outflow"], axis=1, oneway=False
            )
        )
        .assign(
            diff_mannwhitney=(dc.mann_whitney["pvalue"] < 0.05).xs(
                ("inflow", "outflow"), level=["station_1", "station_2"]
            )
        )
        .assign(
            diff_wilcoxon=(dc.wilcoxon["pvalue"] < 0.05).xs(
                ("inflow", "outflow"), level=["station_1", "station_2"]
            )
        )
        .assign(
            diff_symbol=lambda df: wqio.utils.symbolize_bools(
